...
import numpy as np
import torch
from torchvision.transforms import Compose, Resize, CenterCrop, ToTensor, Normalize, RandomRotation, RandomResizedCrop, RandomCrop, RandomHorizontalFlip
try:
...
# define training transform and validation transform used in UDA setting
def get_train_transform(resizing='default', scale=(0.08, 1.0), ratio=(3. / 4., 4. / 3.), random_horizontal_flip=True,
    ...
    if resizing == 'default':
        transform = Compose([
            Resize(256),
            RandomResizedCrop(224, scale=scale, ratio=ratio)
        ])
...
# namedtuple is a special tuple which can be indexed by name.
...
def save_data(data_dir='./data/dataset_32x32', save_file='./data/dataset_32x32.mat', target_size=(32, 32)):
    Xs, Ys = [], []
...
def load_transform(data):
    if data == 'mnist' or data == 'portraits':
        transform = None
    return transform

def data_property(data):
    # return the class number and the image size of the training dataset
    class_num = {
        'mnist': 10,
        'portraits': 2,
    }
    ...
    return class_num[data], image_size[data]

def load_data(dataset, d = 100):
    if dataset == 'mnist':
        (src_tr_x, src_tr_y, src_val_x, src_val_y, inter_x, inter_y, dir_inter_x, dir_inter_y,
            trg_val_x, trg_val_y, trg_test_x, trg_test_y) = rotated_mnist_60_data_func()
    elif dataset == 'mnist_dialing':
        (src_tr_x, src_tr_y, src_val_x, src_val_y, inter_x, inter_y, dir_inter_x, dir_inter_y,
            trg_val_x, trg_val_y, trg_test_x, trg_test_y) = rotated_mnist_60_dialing_ratios_data_func()
    elif dataset == 'portraits':
        (src_tr_x, src_tr_y, src_val_x, src_val_y, inter_x, inter_y, dir_inter_x, dir_inter_y,
            trg_val_x, trg_val_y, trg_test_x, trg_test_y) = portraits_data_func()
    elif dataset == 'portraits_more':
        (src_tr_x, src_tr_y, src_val_x, src_val_y, inter_x, inter_y, dir_inter_x, dir_inter_y,
            trg_val_x, trg_val_y, trg_test_x, trg_test_y) = portraits_data_func_more()
    elif dataset == 'portraits_64':
        (src_tr_x, src_tr_y, src_val_x, src_val_y, inter_x, inter_y, dir_inter_x, dir_inter_y,
            trg_val_x, trg_val_y, trg_test_x, trg_test_y) = portraits_64_data_func()
    elif dataset == 'gaussian':
        (src_tr_x, src_tr_y, src_val_x, src_val_y, inter_x, inter_y, dir_inter_x, dir_inter_y,
            trg_val_x, trg_val_y, trg_test_x, trg_test_y) = gaussian_data_func(d)
    src_tr_x = torch.tensor(src_tr_x).permute(0, 3, 1, 2).to(torch.float32)
    src_tr_y = torch.tensor(src_tr_y).to(torch.long)
    src_val_x = torch.tensor(src_val_x).permute(0, 3, 1, 2).to(torch.float32)
    src_val_y = torch.tensor(src_val_y).to(torch.long)
    inter_x = torch.tensor(inter_x).permute(0, 3, 1, 2).to(torch.float32)
    inter_y = torch.tensor(inter_y).to(torch.long)
    dir_inter_x = torch.tensor(dir_inter_x).permute(0, 3, 1, 2).to(torch.float32)
    dir_inter_y = torch.tensor(dir_inter_y).to(torch.long)
    trg_val_x = torch.tensor(trg_val_x).permute(0, 3, 1, 2).to(torch.float32)
    trg_val_y = torch.tensor(trg_val_y).to(torch.long)
    trg_test_x = torch.tensor(trg_test_x).permute(0, 3, 1, 2).to(torch.float32)
    trg_test_y = torch.tensor(trg_test_y).to(torch.long)
